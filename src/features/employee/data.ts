import type { EmployeeType } from "./type";

const portraitPool = [
  "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=256&q=80",
  "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=256&q=80",
  "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&w=256&q=80",
  "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=256&q=80",
  "https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?auto=format&fit=crop&w=256&q=80",
  "https://images.unsplash.com/photo-1504593811423-6dd665756598?auto=format&fit=crop&w=256&q=80",
];

const baseEmployeeDemoData: Omit<EmployeeType, "imageUrl">[] = [
  {
    employeeId: "EMP-001",
    fullName: "Aung Kyaw",
    email: "aung@gmail.com",
    phone: "+95 912345678",
    department: "IT",
    position: "Frontend Developer",
    joiningDate: "2025-01-10",
    basicSalary: 800000,
    status: "Active",
  },
  {
    employeeId: "EMP-002",
    fullName: "Htet Myat Aung",
    email: "htet@gmail.com",
    phone: "+95 912300001",
    department: "HR",
    position: "HR Officer",
    joiningDate: "2024-12-01",
    basicSalary: 600000,
    status: "Active",
  },
  {
    employeeId: "EMP-003",
    fullName: "Su Su Win",
    email: "susu@gmail.com",
    phone: "+95 912300002",
    department: "Finance",
    position: "Accountant",
    joiningDate: "2023-08-15",
    basicSalary: 750000,
    status: "Active",
  },
  {
    employeeId: "EMP-004",
    fullName: "Kyaw Kyaw Min",
    email: "kyaw@gmail.com",
    phone: "+95 912300003",
    department: "Marketing",
    position: "Marketing Executive",
    joiningDate: "2022-05-20",
    basicSalary: 650000,
    status: "Active",
  },
  {
    employeeId: "EMP-005",
    fullName: "May Thu Hlaing",
    email: "maythu@gmail.com",
    phone: "+95 912300004",
    department: "Sales",
    position: "Sales Manager",
    joiningDate: "2021-03-12",
    basicSalary: 900000,
    status: "Active",
  },
  {
    employeeId: "EMP-006",
    fullName: "Zaw Min Tun",
    email: "zaw@gmail.com",
    phone: "+95 912300005",
    department: "IT",
    position: "Backend Developer",
    joiningDate: "2024-02-01",
    basicSalary: 850000,
    status: "Active",
  },
  {
    employeeId: "EMP-007",
    fullName: "Ei Ei Phyu",
    email: "eiei@gmail.com",
    phone: "+95 912300006",
    department: "Customer Support",
    position: "Support Specialist",
    joiningDate: "2023-06-10",
    basicSalary: 500000,
    status: "Active",
  },
  {
    employeeId: "EMP-008",
    fullName: "Min Min Oo",
    email: "minmin@gmail.com",
    phone: "+95 912300007",
    department: "Operations",
    position: "Operations Officer",
    joiningDate: "2022-09-18",
    basicSalary: 700000,
    status: "Active",
  },
  {
    employeeId: "EMP-009",
    fullName: "Thandar Htun",
    email: "thandar@gmail.com",
    phone: "+95 912300008",
    department: "Finance",
    position: "Payroll Officer",
    joiningDate: "2024-01-05",
    basicSalary: 650000,
    status: "Active",
  },
  {
    employeeId: "EMP-010",
    fullName: "Aye Chan Ko",
    email: "ayechan@gmail.com",
    phone: "+95 912300009",
    department: "IT",
    position: "UI/UX Designer",
    joiningDate: "2023-11-22",
    basicSalary: 720000,
    status: "Active",
  },
  {
    employeeId: "EMP-011",
    fullName: "Nandar Oo",
    email: "nandar@gmail.com",
    phone: "+95 912300010",
    department: "Marketing",
    position: "Content Creator",
    joiningDate: "2022-07-14",
    basicSalary: 580000,
    status: "Active",
  },
  {
    employeeId: "EMP-012",
    fullName: "Kyaw Zin Lin",
    email: "zinlin@gmail.com",
    phone: "+95 912300011",
    department: "Sales",
    position: "Sales Executive",
    joiningDate: "2023-02-28",
    basicSalary: 620000,
    status: "Active",
  },
  {
    employeeId: "EMP-013",
    fullName: "Pyae Sone Aung",
    email: "pyae@gmail.com",
    phone: "+95 912300012",
    department: "IT",
    position: "DevOps Engineer",
    joiningDate: "2021-10-01",
    basicSalary: 950000,
    status: "Active",
  },
  {
    employeeId: "EMP-014",
    fullName: "Cherry Myint",
    email: "cherry@gmail.com",
    phone: "+95 912300013",
    department: "HR",
    position: "Recruiter",
    joiningDate: "2024-03-10",
    basicSalary: 600000,
    status: "Active",
  },
  {
    employeeId: "EMP-015",
    fullName: "Win Ko Ko",
    email: "winkoko@gmail.com",
    phone: "+95 912300014",
    department: "Operations",
    position: "Logistics Coordinator",
    joiningDate: "2022-12-12",
    basicSalary: 680000,
    status: "Active",
  },
  {
    employeeId: "EMP-016",
    fullName: "Mya Mya Aung",
    email: "mya@gmail.com",
    phone: "+95 912300015",
    department: "Customer Support",
    position: "Call Center Agent",
    joiningDate: "2023-09-01",
    basicSalary: 480000,
    status: "Active",
  },
  {
    employeeId: "EMP-017",
    fullName: "Tun Tun Naing",
    email: "tun@gmail.com",
    phone: "+95 912300016",
    department: "Finance",
    position: "Financial Analyst",
    joiningDate: "2021-06-06",
    basicSalary: 880000,
    status: "Active",
  },
  {
    employeeId: "EMP-018",
    fullName: "Shwe Sin Htet",
    email: "shwe@gmail.com",
    phone: "+95 912300017",
    department: "Marketing",
    position: "SEO Specialist",
    joiningDate: "2024-04-01",
    basicSalary: 610000,
    status: "Active",
  },
  {
    employeeId: "EMP-019",
    fullName: "Aung Ko Oo",
    email: "aungoo@gmail.com",
    phone: "+95 912300018",
    department: "IT",
    position: "Mobile Developer",
    joiningDate: "2023-05-17",
    basicSalary: 830000,
    status: "Active",
  },
  {
    employeeId: "EMP-020",
    fullName: "Khin Thazin",
    email: "thazin@gmail.com",
    phone: "+95 912300019",
    department: "HR",
    position: "Training Coordinator",
    joiningDate: "2022-01-25",
    basicSalary: 590000,
    status: "Inactive",
  },
];

export const employeeDemoData: EmployeeType[] = baseEmployeeDemoData.map(
  (employee, index) => ({
    ...employee,
    imageUrl: portraitPool[index % portraitPool.length],
  }),
);
