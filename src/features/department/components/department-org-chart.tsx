...
function OrgCard({
  ...
  return (
    <div
      className={cn(
        "absolute rounded-2xl border bg-white px-3 pb-3 pt-7 text-center shadow-[0_8px_24px_rgba(15,23,42,0.08)]",
        theme.card,
      )}
      style={{
        left: node.x,
    ...
    >
      <div className="absolute left-1/2 top-0 flex -translate-x-1/2 -translate-y-1/2 justify-center">
        <Avatar
          size="lg"
          className={cn(
            "size-12 border-2 border-white bg-white shadow-sm ring-2",
            theme.accent,
          )}
        >
  ...
  return (
    <div className="max-w-full min-w-0">
      <div
        ref={viewportRef}
        className={cn(
          "relative h-[calc(100vh-13rem)] max-h-[900px] min-h-[560px] max-w-full overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-b from-slate-50 via-white to-slate-50",
      ...
      >
        <div className="absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-4 p-4">
          <div className="rounded-2xl border bg-white/90 px-4 py-2 text-sm text-muted-foreground shadow-sm backdrop-blur">
            Scroll to zoom and drag to pan
          </div>
          <div className="flex items-center gap-2 rounded-2xl border bg-white/90 p-2 shadow-sm backdrop-blur">
            <Button
              type="button"
              variant="outline"
              size="icon-sm"
            ...
            <div className="min-w-16 rounded-md border bg-background px-3 py-1 text-center text-sm font-medium">
              {zoomLabel}
            </div>
            <Button
              type="button"
